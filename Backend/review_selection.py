...
def selecionar_melhores_reviews_distintas(
    ...
    for appid, ranking in tqdm(pagerank_resultado.items(), desc="Seleção de reviews distintas"):
        # Recupera a lista de vetores e índice para esse jogo
        lista_completa = reviews_por_jogo.get(appid, [])
        # Mapeia índice -> vetor para acesso rápido
        ...
        for item in ranking:
            ...
            if vetor_atual is None:
                continue  # caso raro, não deve ocorrer

            # Verifica se é muito similar a alguma já selecionada
            muito_similar = False
            for sel in selecionadas:
                vetor_sel = idx_para_vetor[sel["index"]]
                sim = _cosine_similarity(vetor_atual, vetor_sel)
...
def salvar_melhores_reviews_json(
    ...
    Parâmetros:
        ...
        appid_para_jogo: dict {appid: nome_do_jogo}
    """
    resultado_final = []
